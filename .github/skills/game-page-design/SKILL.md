...
- 装备区真实结构是一个父区域 `player.equipArea`，槽位是该区域的固定位置：`equipArea.slots[N]` 保存槽位标签/容量/空槽渲染元数据，`equipArea.cards[N]` 保存该位置的卡牌或 `null`。旧全量同步数据中的 `equipSlots` / `childAreas` 只在恢复边界兼容读取，运行时不要再挂 `player.equipSlots` 或把槽位解析成 child Area。新逻辑应通过 `Models.getEquipSlots()`、`getEquipSlot()`、`getSlotCards()`、`getDefaultEquipSlotIndex()`、`getAreaLocationPath()`、`getCardLocationPath()` 或 `resolveAreaLocationByPath()` 处理槽位。
- 区域路径使用统一格式：`pile`、`discardPile`、`treatmentArea`、`player:N:hand`、`player:N:judgeArea`、`player:N:equip`；槽位等区域内位置使用 location path，如 `player:N:equip:slot:M`。
