...
description: "Use when: 理解或修改动态 HTML 词元系统、技能内容标签语义、术语/卡牌/角色/技能替换、AI_DATA_CONTEXT、compression、replace、summon、tokens admin、词元面板、权限日志、单击详情、双击跳转、高亮。"
...
- 排查替换未生效、重复替换、单击详情、双击跳转、高亮、词元日志或前往按钮问题。
...
- `utils.js` 提供 JSON 缓存、单击词元详情入口和双击滚动高亮绑定。
...
- 所有替换后元素应保留悬停高亮、单击打开词元详情和双击跳转能力，新增类型要接入 `bindTokenDetailOpen`、`bindDblclickAndHighlight` 或等效路径。
- 悬浮备注已全站压制；`LoreTooltip` 仅保留无显示兼容层，不能作为可见功能入口。
...
6. 修改替换结果的 DOM 结构时，检查 hover、高亮、单击详情、双击跳转、主题色、触摸选择和压缩展开。
