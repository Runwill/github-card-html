...
- 小型碎收益应攒成一个有一定规模、可统一验证和汇报的批次；不要每清掉一两处就结束本轮。用户提出的“约 200 行收益”是汇报节奏参考，不是硬门槛；若某批只有约百来行 diff、净收益不大但风险很低，可以连续做两小批再合并算作一轮；若这种合并轮仍属于低风险、用户可见应无变化的整理，也可以再连续两轮合并为一个更大的汇报单位。低收益但能降低复杂度、减少重复判断或修正 owner 的改动仍应顺手做，除非继续修改会跨入更高风险或需要用户确认。
...
7. 规则沉淀：如果清理中发现可复用的反模式、审查口径或旧规则已经误导实现，当轮更新相关 skill、`.github/copilot-instructions.md`、`ARCHITECTURE.md` 或对应项目 `.md`；优先改写或删除过时条目，不只追加新段落。仓库长期规则不要只写入 AI 长期记忆。
