...
5. 圆角使用全局 `--radius-*` token；小控件和缩放组件不要保留固定像素圆角，头像和药丸标签使用 `--radius-full` / `--radius-pill`。
6. 子元素字号默认 `inherit`，只有建立新缩放层级时才重新声明。
7. `html.force-landscape` 中只重定义 token，按视觉轴向进行 `vh` 和 `vw` 互换。
8. 固定高度只用于确实需要屏幕边界的容器；内容区域优先自然撑开，必要时只设置视口安全上限。
9. 删除已经失效的旧定义、重复断点和尺寸补丁，避免在后面追加覆盖层。
...
- 不让会随窗口缩小的控件继续使用固定像素圆角，避免小屏下过度圆润。
