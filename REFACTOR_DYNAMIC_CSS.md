...
> **更新日期**: 2026-04-30
> **本轮目标**: 在进入对局页大块迁移前，先清理低风险的固定尺寸和残留断点。

### 10.1 已完成范围

| 组件 | 状态 | 完成内容 |
|---|---|---|
| 权限筛选下拉 | ✅ 已完成 | `style/custom_select.css` 中日志筛选下拉的宽度、高度、字号改为消费词元面板动态 token |
| 登录页 | ✅ 已完成 | `style/login.css` 删除 480px 宽度断点；容器宽度、面板字号、内距、语言按钮偏移改为连续缩放 |
| 页面面板 | ✅ 已完成 | `.panel--page` 接入登录页字号层级，padding 改为 em |
| 加载页 | ✅ 已完成 | `style/page_loading.css` 以全站根字号建立加载容器字号；标题、进度条、间距和状态文字按标准窗口比例联动缩放，小屏进度条按最长标题宽度兜底，标题字距按真实条宽收敛 |
| 主题零散尺寸 | ✅ 已完成 | 将池占位高度、审核头像尺寸、tokens / permissions 面板 header 高度引用改为动态 token |
| 对局基础牌区 | ✅ 已完成 | `style/media/cards.css`、`processing_area.css`、`footer.css` 删除尺寸断点，牌尺寸、牌面字号、牌堆列宽、桌面三列间距、处理区标签、牌堆查看器留白、检查器高度和页脚改为连续 token |
| 对局控制按钮 | ✅ 已完成 | `style/media/game_controls.css` 删除 1024px 与紧凑视口尺寸断点，按钮宽度、内距、字号和高度改为连续 token，并补齐强制横屏下的等效缩放 |
| 对局角色摘要 | ✅ 已完成 | `style/media/summary_roles.css` 删除多档尺寸断点，角色头像、摘要内距、名字号、徽章和详情按钮改为连续 token |
| 对局高度链 | ✅ 已完成 | 修正全局 flex 高度链和对局顶部区域伸缩基准，避免 947×613 等中等窗口下当前角色面板被顶出视口 |
| 对局无滚动布局 | ✅ 已完成 | `main_perspective.css`、`area_windows.css`、`move_log.css` 改为顶部/底部同步连续缩放；对局页顶部区域和移动记录不再创建滚动容器 |
| 对局状态徽章 | ✅ 已完成 | HP、手牌数、判定角标、状态按钮行和计时条接入连续缩放 token，移除静态下限对布局的挤压 |
| 对局日志侧栏 | ✅ 已完成 | 移除 1024px 下日志面板置顶拉长的布局切换，并清理 `online_room.css` 中残留的对局视图换列规则；侧栏宽度下限、面板最小宽度和计时条换行均接入动态缩放，日志栏始终保持正常对局页右侧栏结构且不挤压牌区 |
| 对局视图遗留控件 | ✅ 已完成 | 当前角色面板内距和阴影、区域水印、查看器水印、装备槽标签、检查器浮层、面包屑、角色面板标题、牌面阴影和平铺牌距接入连续缩放；当前角色留白随牌宽更快收敛，角色信息换行后进一步收紧且强制横屏保持一致，拥挤时保留受控露边重叠，1912×948 基准值保持不变 |

### 10.2 后续重点

- `style/media/` 剩余断点主要用于结构切换；后续重点转向消费侧固定尺寸和对局状态变化下的真实内容验收。
- `game/styles/` 剩余重点转向在线房间辅助区和少量上下文菜单固定尺寸，需结合真实对局流程继续验收。