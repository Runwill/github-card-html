...
| 对局基础牌区 | ✅ 已完成 | `style/media/cards.css`、`processing_area.css`、`footer.css` 删除尺寸断点，牌尺寸、牌面字号、牌堆列宽、桌面三列间距、处理区标签、牌堆查看器留白和页脚改为连续 token |
| 对局控制按钮 | ✅ 已完成 | `style/media/game_controls.css` 删除 1024px 与紧凑视口尺寸断点，按钮宽度、内距、字号和高度改为连续 token |
...
- `game/styles/` 仍有角色面板标题、面包屑、部分在线房间辅助区等固定尺寸，需在 `style/media/` token 梳理后再迁。