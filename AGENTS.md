...
**何时必须停顿**：当修复方案确定并通过验证后，**立即停下来**执行以下判断，不继续处理下一个问题：

1. **判断是否为"非平凡 bug"**（满足任一条件即为非平凡）：
   - 排查路径走过 2 个以上错误假设才定位到真实根因
   - 根因在架构层（如 CSS 优先级、设计语义、模块边界）而非表面问题
   - 用户提供了关键排查方向，而 agent 此前未自行想到
   - 涉及 `!important` 的使用或删除
   - 涉及选择器优先级、主题覆盖、样式迁移等技术债

2. **若是非平凡 bug**：
   - 立即阅读 `.github/agent-experience/bug-patterns.md` 和 `missed-cases.md`
   - 判断是否需要补录经验条目
   - 若需要补录，**在继续下一个任务前完成补录**
...
**为什么需要强制停顿**：
- 避免"修完就忘"：定位完成时是经验最鲜活的时候，延迟判断会丢失关键细节
- 避免低估问题严重性：表面简单的样式问题可能涉及架构层面的优先级冲突
- 避免等用户提醒：经验库自检是 agent 的主动职责，不是被动响应
