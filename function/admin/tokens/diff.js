    ...
    const walk = (o, n, p) => {
      ...
      if (Array.isArray(o) || Array.isArray(n)) {
        ...
        for (let i = 0; i < len; i++) {
          ...
          if (i >= na.length) { addDel(op, oa[i]); }
          else if (i >= oa.length) {
            if (isPrimitive(na[i])) addSet(op, undefined, na[i]); else { walk(undefined, na[i], op); }
          } else {
            if (isPrimitive(oa[i]) || isPrimitive(na[i])) {
              if (JSON.stringify(oa[i]) !== JSON.stringify(na[i])) addSet(op, oa[i], na[i]);
