...
(function(){
  ...
  function renderSchemeSelector(curVariants){
      ...
      const schemeText=(idx)=>(((window.t&&window.t('tokens.scheme.label'))||'方案 {index}').replace('{index}', String(idx)));
      const countText=(count)=>(((window.t&&window.t('tokens.scheme.count'))||'{count} 项').replace('{count}', String(count)));
      const groupHtml = `<div class="tokens-scheme__group">${curVariants.map((v,i)=>{ const idx=i+1; const selCls=i===0? ' is-selected': ''; const tab=i===0? '0':'-1'; const pressed=i===0? 'true':'false'; return `<div class="tokens-scheme__btn${selCls}" tabindex="${tab}" role="button" aria-pressed="${pressed}" data-index="${i}"><span class="name">${esc(schemeText(idx))}</span><span class="count">${esc(countText(v.count))}</span></div>`; }).join('')}</div>`;
      schemeBox.innerHTML=groupHtml;
      const selectIdx=(idx)=>{ try{ const variantsList= schemeBox.__variants||[]; if(!Number.isFinite(idx) || idx<0 || idx>=variantsList.length) return; const btns= Array.from(schemeBox.querySelectorAll('.tokens-scheme__btn')); btns.forEach((b,i)=>{ const on=i===idx; b.classList.toggle('is-selected', on); b.setAttribute('tabindex', on? '0':'-1'); b.setAttribute('aria-pressed', on? 'true':'false'); }); schemeBox.dataset.selectedIndex= String(idx); const v= variantsList[idx]; if(v){ editor.value= JSON.stringify(v.tpl||{}, null, 2); renderHintsFromVariants(v.tpl, variantsList); } }catch(_){ } };
