...
(function(){
  // 状态
  const state = { data: null, q: '', timer: null, activeType: null };
  // 搜索输入防抖间隔（毫秒）
  ...
  async function renderTokensDashboard(forceReload=false){
    ...
    try{
      ...
      const section=(type,title,items,renderItem)=>{
        const id='sec-'+Math.random().toString(36).slice(2,8); const total=Array.isArray(items)? items.length: 0; const shouldPreOpen= !!state.activeType && state.activeType===type && total>1; const allItemsHtml=(items||[]).map(renderItem).join(''); const collapsedAreaHtml = (total>1)? '': (allItemsHtml || '<div class="tokens-empty">空</div>');
        return `<div class="tokens-section"><div class="tokens-section__header"><div class=\"tokens-section__title\">${title} <span class=\"count-badge\">(${total})</span></div><div class=\"tokens-section__ops\">${canEdit? `<button class=\"btn btn--secondary btn--sm\" onclick=\"tokensOpenCreate('${type}')\">新增</button>`: ''}${total>1? `<button id=\"btn-${id}\" class=\"btn btn--secondary btn--sm expand-btn${shouldPreOpen? ' is-expanded':''}\" aria-expanded=\"${shouldPreOpen? 'true':'false'}\" onclick=\"toggleTokensSection('${id}')\">${shouldPreOpen? '收起':'展开'}</button>`: ''}</div></div><div id="${id}" data-expanded="${shouldPreOpen? '1':'0'}" class="tokens-section__body"><div class="token-list">${collapsedAreaHtml}</div>${total>1? `<div id=\"more-${id}\" class=\"js-more token-list collapsible tokens-section__more${shouldPreOpen? ' is-open':''}\">${allItemsHtml}</div>`: ''}</div></div>`;
      };
  ...
  window.toggleTokensSection = function(baseId){ try{ const root=document.getElementById(baseId); if(!root) return; const btn=document.getElementById('btn-'+baseId); const more=document.getElementById('more-'+baseId) || root.querySelector('.js-more'); if(!more) return; const expanded=root.getAttribute('data-expanded')==='1'; const transitionMs=400; const setBtn=(isOpen)=>{ if(btn){ btn.textContent=isOpen? '收起':'展开'; btn.setAttribute('aria-expanded', isOpen? 'true':'false'); btn.classList.toggle('is-expanded', isOpen); } }; const onEnd=(cb)=>{ let called=false; const handler=()=>{ if(called) return; called=true; more.removeEventListener('transitionend',handler); cb&&cb(); }; more.addEventListener('transitionend', handler, { once:true }); setTimeout(handler, transitionMs+50); }; if(!expanded){ more.style.display='block'; more.classList.add('is-opening'); more.style.height='0px'; void more.offsetHeight; const target=more.scrollHeight; more.style.height=target+'px'; onEnd(()=>{ more.classList.remove('is-opening'); more.classList.add('is-open'); more.style.height='auto'; }); root.setAttribute('data-expanded','1'); setBtn(true); } else { const from=more.scrollHeight; more.style.height=from+'px'; void more.offsetHeight; more.classList.remove('is-open'); more.classList.add('is-closing'); more.style.height='0px'; onEnd(()=>{ more.classList.remove('is-closing'); more.style.display='none'; more.style.height='0px'; }); root.setAttribute('data-expanded','0'); setBtn(false); } }catch(_){} };
  ...
  function enableInlineEdit(rootEl){ if(rootEl.__inlineEditBound) return; rootEl.__inlineEditBound=true; rootEl.addEventListener('click', function(ev){ if(ev.ctrlKey || document.body.classList.contains('ctrl-down')) return; const host= ev.target && ev.target.closest? ev.target.closest('.kv-val'): null; if(!host) return; const target=host; const openEditing = rootEl.querySelector('.kv-val[data-editing="1"]'); if(openEditing && openEditing!==target){ const old=openEditing.getAttribute('data-old-text')||''; openEditing.textContent=old; openEditing.removeAttribute('data-editing'); openEditing.removeAttribute('data-old-text'); openEditing.classList.remove('is-editing','is-saving'); } const path= target.getAttribute('data-path'); if(!path || path.startsWith('_') || path.includes('.__v')) return; const type= target.getAttribute('data-type')||'string'; const tokenCard= target.closest('.token-card[data-coll][data-id]'); if(!tokenCard) return; const coll= tokenCard.getAttribute('data-coll'); const id= tokenCard.getAttribute('data-id'); if(!coll||!id) return;
      ...
      let input; let colorPicker=null; const applyPreview=(val)=>{ try{ if(!tokenCard||!val) return; const col= String(val).trim(); if(!col) return; const tint= computeTint(col); tokenCard.style.setProperty('--token-accent', col); if(tint) tokenCard.style.setProperty('--token-bg', tint); tokenCard.style.borderLeft= `3px solid ${col}`; }catch(_){}};
