...
2. **用户视角描述**：写用户能感知到的变化（"头栏缩放更平滑"），不写怎么实现的（"改用 vw clamp"）
3. **一句话一条**：每条 ≤ 20 字为佳，最长不超过 30 字。不用破折号、分号展开解释
4. **前缀分类**：`新增：` / `优化：` / `修复：`，不用 `added` / `improved` / `fixed`
5. **合并同类项**：同一区域的多项内部重构合并为一条用户可感知的描述；纯内部重构（用户无感知）不写入公告
6. **保留历史日期**：公告同时承担更新时间线记录；历史日期下的内容失效时，应改写为合规的用户视角摘要，不要直接删除该日期条目，除非用户明确要求删除
7. **参考已有公告的语气**，保持一致：
   - ✅ `"优化：头栏响应式缩放优化"`
   - ✅ `"修复：修复横屏模式下头栏布局异常"`
...
3. ✅ 子元素是否通过 `inherit` 获取字号，而非重复声明？
4. ✅ 1912×948 下的渲染值是否与改动前完全一致？
5. ✅ force-landscape token 块是否已将 vh↔vw 互换（而非固定值）？
6. ✅ 继承链注释是否完整（来源、向下传递、特殊尺寸）？
7. ✅ 是否清理了冗余的 min/max、!important、重复 font-size？
