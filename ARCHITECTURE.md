...
每次做完**用户可见的变更**后，必须在 `base/announcements.json` 最新日期条目中追加变更项，或新建一条公告。
...
当某个页面/组件需要在不同视口尺寸下**等比缩放**（而非仅在断点切换），应遵循以下流程和原则。

#### 基准窗口

以 **1738×978**（视口 `innerWidth × innerHeight`）为 100% 参考基准。所有 `clamp()` 的中间值按此基准计算百分比。

#### 完整属性清单——每次动态化一个组件时必须覆盖

对一个 UI 组件做动态缩放时，**必须同时处理以下全部维度**，遗漏任何一项都会导致比例失调：

| 维度 | 说明 | 典型属性 |
|---|---|---|
| **高度** | 容器/控件的整体高度 | `height`、`max-height`、`min-height` |
| **纵向内距** | 内容与容器边缘的上下间距 | `padding-top`、`padding-bottom` |
| **横向内距** | 内容与容器边缘的左右间距 | `padding-left`、`padding-right` |
| **外距** | 容器之间的间距 | `margin`、`gap` |
| **字号** | 文本大小 | `font-size` |
| **最小宽度/弹性基准** | 搜索框等弹性元素的 flex-basis / min-width | `min-width`、`flex-basis` |
| **布局控制** | flex-wrap、width 等在断点处切换的属性 | `flex-wrap`、`width` |

> **反例提醒**：只动态化了容器高度和间距，但字号仍用固定 `--fs-md`，会导致小视口下文字与容器比例失调；只动态化了高度但 padding 仍然固定，会导致横屏下内距占比过大。

#### token 命名与定义

```css
/* :root 中定义，消费者通过 var() + fallback 引用 */
--tokens-input-h:  clamp(最小值, 基准/978*100 vh, 最大值);   /* 纵向维度 → vh */
--tokens-input-px: clamp(最小值, 基准/1738*100 vw, 最大值);  /* 横向维度 → vw */
--tp-fs-md:        clamp(最小值, 基准/978*100 vh, 最大值);   /* 字号 → vh（跟高度走） */
```

- **纵向属性**（高度、纵向 padding、gap、margin-top/bottom、font-size）使用 **`vh`**
- **横向属性**（横向 padding、min-width、flex-basis）使用 **`vw`**
- **混合 shorthand**（如 `padding: Y X`）拆成两个 clamp：`clamp(…vh) clamp(…vw)`
- 命名前缀与作用域对应：`--tokens-*`（词元页）、`--approval-*`（权限页审批行）、`--tp-fs-*`（共享字号）

#### 消费者引用模式

```css
/* 始终提供 fallback = 原始静态值，保证 token 未定义时无损降级 */
font-size: var(--tp-fs-md, var(--fs-md));
padding:   var(--tokens-input-py, var(--space-sm)) var(--tokens-input-px, var(--space-md));
height:    var(--tokens-input-h, 36px);
```

#### force-landscape 覆盖

横屏模式下 CSS `vh` = 手机物理高度（~675px），**不是**旋转后的视觉高度（~371px），因此 `clamp()` 算出的值偏大。必须在 `html.force-landscape { }` token 块中用**固定值**覆盖：

```css
html.force-landscape {
  --tokens-input-h: 22px;          /* 而非 clamp 的 ~25px */
  --tp-fs-md: 12px;                /* 而非 clamp 的 ~9.6px→但实际 min 兜底到 11px */
}
```

同时注意：如果某个属性在 `@media (max-width: 480px)` 中被 `flex-wrap: wrap` 等改变了布局，force-landscape 的 token 块中需要显式重置（`--tokens-toolbar-wrap: nowrap`），因为 CSS 视口宽 ≈ 371px 会命中该媒体查询，但视觉宽度 ~675px 实际不需要换行。

#### 检查清单（每次提交前）

1. ✅ 组件的**全部 7 个维度**（高度、纵距、横距、外距、字号、弹性尺寸、布局控制）是否都已 token 化？
2. ✅ 1738×978 下的渲染值是否与改动前完全一致（clamp 中间值 = 原始 px）？
3. ✅ force-landscape token 块是否已添加对应的固定值覆盖？
4. ✅ `@media` 断点中被改变的布局属性是否已 token 化，且 force-landscape 中已重置？
5. ✅ 消费者是否都使用了 `var(--token, fallback)` 模式，不会因 token 缺失而破坏样式？
