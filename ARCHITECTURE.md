...
- **程序特殊**：程序面板细分为 `normal` / `debug`，调试侧栏开关会刷新帮助内容
- **草稿特殊**：草稿面板细分为 `editor` / `relations` 两个子视图
...
#### ❌ `:is()` 内包含 ID 选择器导致特异度泄漏

在共享 owner CSS（如 `ui_controls.css`）中使用 `:is(.ui-field, #login-form input)` 时，`:is()` 的特异度取括号内**最高值**。`#login-form input` 含 ID 选择器 → 整个 `:is()` 的特异度变成 (1,0,1)，所有 `.ui-field` 消费者都获得 ID 级特异度，压过后续 class 级覆盖规则（(0,5,0) 也打不赢 (1,0,1)）。

**✅ 正确做法**：把含 ID/元素选择器的参数拆到 `:is()` 外面，用逗号分隔：

```css
/* ❌ 错误：ID 污染整个 :is() */
:is(.ui-field, #login-form input, .custom-select__trigger) { }

/* ✅ 正确：ID 独立，不污染 class 级 */
.ui-field,
:is(.modal-form input, .custom-select__trigger),
#login-form input { }
```

排查方法：DevTools 中对比"被划掉的规则"和"生效规则"的特异度 (a,b,c)，若 class 级规则被莫名压过，追溯到 `:is()` 内查找是否有 ID 或元素选择器。
...
| `panel_term.html` | "程序"面板：主容器；正文优先由 `program_panel.v2` 树数据生成 |
